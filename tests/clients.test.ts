...
import type { Client, ClientsPage, User, UsersPage } from "../src/sdk";
...
describe("Clients", () => {
  ...
  test("List client users should list users linked to a client", async () => {
    fetchMock.mockResponseOnce(JSON.stringify(usersPage));

    const response = await sdk.clients.ListClientUsers(
      clientId,
      queryParams,
      domainId,
      token
